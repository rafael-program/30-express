...
export default function OrderDetailPage() {
  ...
  const fetchOrder = async () => {
    ...
    try {
      ...
      const mappedData = {
        ...data,
        customer_address: data.delivery_address || data.customer_address || 'Endereço não informado'
      };
      ...
      if (mappedData) {
        setEditData({
          ...
          delivery_time: mappedData.delivery_time || '',
        ...
        if (mappedData.delivery_agent_id) {
          setSelectedAgentId(mappedData.delivery_agent_id);
        }
      }
    } catch (error: any) {
  ...
  const assignDeliveryAgent = async () => {
    ...
    try {
      ...
      await supabase.from('order_logs').insert({
        order_id: order.id,
        status: 'out_for_delivery',
        description: `Entregador atribuído: ${deliveryAgents.find(a => a.id === selectedAgentId)?.full_name}`
      });
    ...
    return (
      <div className="flex items-center justify-center h-96">
        <div className="text-center">
          ...
          <p className="mt-4 text-[#2d6a4f] font-medium">Carregando pedido...</p>
        </div>
      </div>
    );
  }
              ...
              {order.delivery_agent_id ? (
                <div className="p-4 bg-green-50 rounded-xl border border-green-200">
                  ...
                  <div className="mt-2 space-y-1 text-sm">
                    <p><strong>Nome:</strong> {order.delivery_agent?.full_name || 'Não disponível'}</p>
                    <p><strong>Telefone:</strong> {order.delivery_agent?.phone || '-'}</p>
                    <p><strong>Veículo:</strong> {order.delivery_agent?.vehicle || '-'}</p>
                  </div>
                </div>
              ) : (
                <div className="space-y-3">
                  ...
                  <button
                    ...
                    disabled={!selectedAgentId || assigning || order.status === 'delivered' || order.status === 'cancelled'}
                ...
                order.items.map((item) => (
                  <div key={item.id} className="flex items-center justify-between py-3 border-b border-gray-100 last:border-0">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-medium text-gray-800">{item.product_name || 'Produto'}</p>
                        <p className="text-sm text-gray-500">
                          {formatCurrency(item.unit_price || item.price || 0)} / unidade
                        </p>
                      </div>
                    </div>
            ...
            {isEditing ? (
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <input
                    type="datetime-local"
                    value={editData.delivery_time ? new Date(editData.delivery_time).toISOString().slice(0, 16) : ''}
                ...
                <button
                  onClick={saveChanges}
                  disabled={saving}
                  className="w-full py-2 bg-[#2d6a4f] text-white rounded-xl hover:bg-[#1b4332] transition font-medium flex items-center justify-center gap-2"
                >
                  {saving ? (
                    <>
                      <Loader2 className="w-4 h-4 animate-spin" />
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span className="text-gray-600">{order.customer_name || 'Cliente'}</span>
                </div>
                <div className="flex items-center gap-2 text-sm">
                ...
                <div className="flex items-start gap-2 text-sm">
                  ...
                  <span className="text-gray-600">{order.customer_address || order.delivery_address || '-'}</span>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-3xl shadow-lg p-6">
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="capitalize">{getPaymentLabel(order.payment_method)}</span>
              </div>
              <div className="flex justify-between text-sm">
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 text-center">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-bold text-[#2d6a4f]">QR Code do Pedido</h2>
              <button
            ...
            <div className="p-4 bg-white border-2 border-dashed border-gray-200 rounded-xl">
              <img
                src={`https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${order.qr_code || order.order_number}`}
                alt="QR Code"
            ...
            <button
              onClick={() => {
                ...
                link.href = `https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${order.qr_code || order.order_number}`;
                link.click();
              }}
